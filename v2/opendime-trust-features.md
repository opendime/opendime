...
For regular people who just want to verify a specific Opendime,
you'll only need to read the first section.
...
them as it run=s, but that is slow and requires an Internet connection.

    - `pip install pycoin`
    - `pip install ecdsa`

We require version `0.70` or later for pycoin. Use `virtualenv`
...
Here's what it will look like for a typical device (many of the
...
code from a trusted source to verify an Opendime. That could be the
same file from another Opendime that you know is good. To test between
two units, provide the path to the *other* Opendime as the first argument:
...
and run directly from that checkout, under `sealed/advanced/trustme.py`.
...
device serial number from your operating system somehow?  (In this example,
that's SPX6ESSUJVGVCIBAEBDTMDAK74). The second part of the
serial number is the serial number laser-etched into the ATECC508A.
...
(Unfortunately, the detailed datasheet is
covered by an NDA, so we cannot get into all the details.)
...
- NIST Standard P256 Elliptic Curve Support (cannot do other curves).
...
- pick 20 bytes random nonce (160 bits)
...
Certificate: XXX REPLACE ME XXX
    Data:
        Version: 3 (0x2)
        Serial Number:
            91:45:b1:37:c7:83:de:83
        Signature Algorithm: sha256WithRSAEncryption
        Issuer: CN=Batch #4 CA
        Validity
            Not Before: Feb  1 00:00:00 2017 GMT
            Not After : Jan  1 00:00:00 2037 GMT
        Subject: serialNumber=SPX6ESSUJVGVCIBAEBDTMDAK74+2155ccbebab2, CN=Opendime
        Subject Public Key Info:
            ...
            EC Public Key:
                pub: 
                    04:8f:92:c6:22:df:9b:3a:03:7a:2c:94:c4:91:91:
                    e7:0f:5d:f2:cb:01:c0:38:58:60:9e:f5:2b:48:65:
                    ec:b5:f1:94:7b:3c:43:ac:9c:4b:80:13:ae:3e:7c:
                    8c:db:fc:f5:70:8a:db:69:03:56:46:b7:17:ff:45:
                    33:11:5c:3d:03
                ASN1 OID: prime256v1
        X509v3 extensions:
            X509v3 Authority Key Identifier: 
                keyid:1E:46:5E:E9:58:E5:72:4F:32:95:5B:54:D2:D8:B9:E2:D6:B9:05:A0    XXX

            X509v3 Basic Constraints: 
                CA:FALSE
            X509v3 Subject Key Identifier: 
                75:C3:14:51:F3:F0:DA:30:E0:46:7C:EB:8C:33:C9:A5:AD:C7:A4:E5
            X509v3 Key Usage: critical
                Digital Signature
    Signature Algorithm: sha256WithRSAEncryption
        1c:ff:f4:5e:21:9e:bf:ef:b4:d9:75:c4:d5:68:f9:d7:a0:f4:
        ea:c5:29:ac:6c:b0:3f:46:b5:b7:b9:a7:20:17:58:36:07:8d:
        c2:be:62:45:fe:68:a5:9e:ed:36:37:e4:bd:d7:ed:9c:a9:85:
        7a:31:76:03:2e:2f:7b:7b:c3:51:e9:73:b1:e5:75:00:0a:56:
        41:b2:41:74:8a:0f:27:c7:a7:c1:7e:96:f1:1d:7b:83:fd:cc:
        7b:28:57:12:97:24:c5:84:93:78:d5:bc:41:04:50:20:46:0a:
        9b:16:b0:4f:3a:e0:a4:4e:4a:f1:e8:0b:37:d5:b8:81:20:c8:
        94:c7:1a:5c:80:cd:fe:58:8b:8d:16:ad:cd:a5:a3:e5:04:94:
        fb:6f:d8:e7:ae:30:ef:39:26:18:32:05:60:51:b3:94:b9:58:
        57:5c:cf:e3:bc:cf:d2:7d:64:56:63:e4:1a:2b:62:e1:10:e1:
        3b:4c:5e:ef:ef:56:9c:a2:9e:a8:1b:48:bd:89:61:f3:89:8d:
        ca:f8:c5:bf:48:a2:87:ca:6f:df:2a:44:65:f9:20:8a:aa:aa:
        af:66:2b:9d:61:f1:14:cf:bc:1f:34:21:0d:22:56:d6:e3:88:
        f0:51:bc:e1:31:e1:c9:11:36:b2:c9:39:80:6b:89:f7:ea:9e:
        9f:5e:79:0e
```
...
-----BEGIN PUBLIC KEY-----
MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEj5LGIt+bOgN6LJTEkZHnD13yywHA        XXX REPLACE ME XXX
OFhgnvUrSGXstfGUezxDrJxLgBOuPnyM2/z1cIrbaQNWRrcX/0UzEVw9Aw==
-----END PUBLIC KEY-----
```
...
part of the signature. This means it is impossible for even us to
exactly clone an Opendime. We can only make new original ones.

## Hackers Welcome

Since any _real hacker_ will discover the following in a few minutes, we can share
this information:
